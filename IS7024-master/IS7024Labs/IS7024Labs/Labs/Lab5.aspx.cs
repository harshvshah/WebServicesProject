...
namespace IS7024Labs.Labs
{
    public partial class Lab5 : System.Web.UI.Page
    {
        ...
        {
            HttpWebRequest request = CreateWebRequest(@"https://maps.googleapis.com/maps/api/place/autocomplete/xml?key=AIzaSyBJs3euHjvXV3_oTlDRbRyhFsq9NcCzoxo&input=" + Address);
            string AutoComplete = "";
            using (WebResponse response = request.GetResponse())
            {
                using (StreamReader rd = new StreamReader(response.GetResponseStream()))
        ...
        {
            HttpWebRequest request = CreateWebRequest(@"https://maps.googleapis.com/maps/api/place/details/xml?key=AIzaSyBJs3euHjvXV3_oTlDRbRyhFsq9NcCzoxo&placeid=" + PlaceID);
            string PlaceDetails = "";
            using (WebResponse response = request.GetResponse())
            {
                using (StreamReader rd = new StreamReader(response.GetResponseStream()))
